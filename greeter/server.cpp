...
class GreeterServiceImpl final : public Greeter::Service {
  ...
  // virtual ::grpc::Status SayVector(::grpc::ServerContext* context, const flatbuffers::grpc::Message<HelloRequest>* request, flatbuffers::grpc::Message<VectorReply>* response);

  ::grpc::Status SayVector(
      grpc::ServerContext *context,
      const flatbuffers::grpc::Message<VectorRequest> *request_msg,
      flatbuffers::grpc::Message<VectorReply> *response_msg) override 
  {
    // `flatbuffers::grpc::MessageBuilder` is a `FlatBufferBuilder` with a
    // special allocator for efficient gRPC buffer transfer, but otherwise
    // usage is the same as usual.
    ...
    // We call GetRoot to "parse" the message. Verification is already
    // performed by default. See the notes below for more details.
    const VectorRequest *request = request_msg->GetRoot();

    // Fields are retrieved as usual with FlatBuffers
    std::vector<flatbuffers::Offset<flatbuffers::String> > messages;
    const flatbuffers::Vector<flatbuffers::Offset<flatbuffers::String>>* names = request->names();
    assert(names != nullptr);
    for (const auto& name : *names)
    {
      messages.push_back(mb_.CreateString("Hello, "+name->str()));
    }
    auto hello_offset = CreateVectorReplyDirect(mb_, &messages);
    mb_.Finish(hello_offset);

    // The `ReleaseMessage<T>()` function detaches the message from the
    // builder, so we can transfer the resopnse to gRPC while simultaneously
    // detaching that memory buffer from the builer.
    *response_msg = mb_.ReleaseMessage<VectorReply>();
    assert(response_msg->Verify());
